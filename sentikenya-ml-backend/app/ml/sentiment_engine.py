...
class SentimentEngine:
    ...
    def __init__(
        ...
        self._active_model_name = "rule-based"
    ...
    async def load_model(self):
        ...
        Priority order:
        1. Fine-tuned SentiKenya model (./models/sentikenya-v1/best)
        2. Base HuggingFace model (cardiffnlp/twitter-xlm-roberta-base-sentiment-multilingual)
        3. Rule-based fallback (no transformer)
        """
        import os
        try:
            ...
            finetuned = self.model_config.finetuned_path
            if os.path.isdir(finetuned) and os.path.exists(os.path.join(finetuned, "config.json")):
                load_path = finetuned
                self._active_model_name = "sentikenya-v1-finetuned"
                logger.info(f"Loading fine-tuned SentiKenya model from: {load_path}")
            else:
                load_path = self.model_config.model_name
                self._active_model_name = "xlmr-base-multilingual"
                logger.info(f"No fine-tuned model at {finetuned}, loading base: {load_path}")
            ...
            logger.info(f"Sentiment model loaded: {self._active_model_name}")
        ...
        # ── Load custom BiLSTM model (ensemble member) ──
        try:
            custom_path = self.model_config.custom_model_path
            config_file = os.path.join(custom_path, "config.json")
            if os.path.isdir(custom_path) and os.path.exists(config_file):
                from app.ml.custom_model import CustomSentimentPredictor
                self._custom_predictor = CustomSentimentPredictor.load(custom_path)
                self._custom_loaded = True
                logger.info(f"Custom BiLSTM model loaded from: {custom_path}")
            else:
                logger.info(f"No custom model at {custom_path}, skipping")
        except Exception as e:
    ...
    def _analyze_sheng(self, text: str) -> SentimentResult:
        ...
        # Get custom BiLSTM score (fast, ~1ms)
        custom_score = self._custom_score(text) if self._custom_loaded else None

        # Ensemble blend: lexicon + transformer + custom BiLSTM
        if self._model_loaded:
            model_score = self._transformer_score(text)

            if self._custom_loaded and custom_score is not None:
                # Full ensemble: 3 models
                if self._active_model_name == "sentikenya-v1-finetuned":
                    # Fine-tuned + custom: 20% lexicon, 55% transformer, 25% custom
                    final_score = lexicon_score * 0.20 + model_score * 0.55 + custom_score * 0.25
                    confidence = 0.83
                else:
                    # Base + custom: 30% lexicon, 45% transformer, 25% custom
                    final_score = lexicon_score * 0.30 + model_score * 0.45 + custom_score * 0.25
                    confidence = 0.76
                model_tag = f"sheng-ensemble+{self._active_model_name}+bilstm"
            else:
                # Transformer + lexicon only
                if self._active_model_name == "sentikenya-v1-finetuned":
                    final_score = lexicon_score * 0.25 + model_score * 0.75
                    confidence = 0.80
                else:
                    final_score = lexicon_score * 0.4 + model_score * 0.6
                    confidence = 0.72
                model_tag = f"sheng-hybrid+{self._active_model_name}"

        elif self._custom_loaded and custom_score is not None:
            # Custom + lexicon only (no transformer)
            final_score = lexicon_score * 0.4 + custom_score * 0.6
            confidence = 0.65
            model_tag = "sheng-hybrid+bilstm"
        else:
            final_score = lexicon_score
            confidence = 0.55
            model_tag = "sheng-lexicon-only"
    ...
    def _analyze_swahili(self, text: str) -> SentimentResult:
        ...
        if self._model_loaded:
            model_score = self._transformer_score(text)
            if self._custom_loaded and custom_score is not None:
                # 3-model ensemble: 25% lexicon, 50% transformer, 25% custom
                final_score = combined * 0.25 + model_score * 0.50 + custom_score * 0.25
                confidence = 0.85
                model_tag = f"swahili-ensemble+{self._active_model_name}+bilstm"
            else:
                final_score = combined * 0.3 + model_score * 0.7
                confidence = 0.82
                model_tag = f"swahili-hybrid+{self._active_model_name}"
        elif self._custom_loaded and custom_score is not None:
            final_score = combined * 0.5 + custom_score * 0.5
            confidence = 0.65
            model_tag = "swahili-hybrid+bilstm"
        else:
            final_score = combined
            confidence = 0.60
            model_tag = "swahili-lexicon-only"
    ...
    def _analyze_english(self, text: str) -> SentimentResult:
        """English sentiment — transformer preferred, custom BiLSTM as ensemble."""
        custom_score = self._custom_score(text) if self._custom_loaded else None

        if self._model_loaded:
            transformer_score = self._transformer_score(text)
            if self._custom_loaded and custom_score is not None:
                # Ensemble: 65% transformer, 35% custom BiLSTM
                score = transformer_score * 0.65 + custom_score * 0.35
                confidence = 0.90
                model_tag = f"en-ensemble+{self._active_model_name}+bilstm"
            else:
                score = transformer_score
                confidence = 0.88
                model_tag = self._active_model_name
        elif self._custom_loaded and custom_score is not None:
            score = custom_score
            confidence = 0.70
            model_tag = "bilstm-only"
        else:
            tokens = self.sheng_tokenizer.tokenize(text)
            score = self._lexicon_score(text, tokens)
            confidence = 0.62
            model_tag = "lexicon-only"
    ...
    def _analyze_ensemble(self, text: str, language: LanguageDetection) -> SentimentResult:
        ...
        return SentimentResult(
            ...
            probabilities=self._score_to_probs(final_score),
            model_used=f"ensemble+{self._active_model_name}",
        )
    ...
    def _custom_score(self, text: str) -> float:
        """
        Get sentiment score from custom BiLSTM + Attention model.

        Returns score in [-1, 1] range, or 0.0 if model not loaded.
        Lightweight (~1ms) complement to the transformer (~40ms).
        """
        if not self._custom_loaded or not self._custom_predictor:
            return 0.0

        try:
            return self._custom_predictor.predict_score(text)
        except Exception as e:
