...
class IngestionManager:
    ...
    def __init__(
        ...
        if reddit_client_id:
            ...
            logger.info("[ingestion] Reddit connector registered (OAuth)")
        else:
            # Reddit works without auth (lower rate limits)
            self._connectors["reddit"] = RedditConnector(
                config=self.config,
                poll_interval=180,  # Slower polling without auth
            )
            logger.info("[ingestion] Reddit connector registered (public API)")
        ...
        if not self._connectors:
            logger.warning("[ingestion] No connectors configured — add API keys to enable")
