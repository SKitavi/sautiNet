...
ingestion: IngestionManager = None


@asynccontextmanager
async def lifespan(app: FastAPI):
    """Application startup and shutdown lifecycle."""
    global pipeline, kafka, ipfs, broadcaster, worker, worker_task, ingestion
    ...
    # ── Start social media ingestion ──
    if settings.ENABLE_INGESTION:
        fb_pages = settings.FACEBOOK_PAGES.split(",") if settings.FACEBOOK_PAGES else None
        ...
        logger.info(f"Ingestion active: {', '.join(ingestion.active_connectors) or 'waiting for tasks...'}")

    # Configure API routes with all dependencies
    configure_routes(pipeline, worker, broadcaster, ipfs, ingestion)
