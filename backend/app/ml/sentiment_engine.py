...
class SentimentEngine:
    ...
    def _analyze_sheng(self, text: str) -> SentimentResult:
        ...
        if self._model_loaded:
            ...
            # Dynamic blend: fine-tuned model gets more weight (it learned Sheng)
            if self._active_model_name == "sentikenya-v1-finetuned":
                # Fine-tuned: 25% lexicon safety net, 75% model
                final_score = lexicon_score * 0.25 + model_score * 0.75
                confidence = 0.80
            else:
                # Base model: 40% lexicon (Sheng-specific), 60% model (general)
        ...
        return SentimentResult(
            ...
            model_used=f"sheng-hybrid+{self._active_model_name}",
        )
    ...
    def _analyze_swahili(self, text: str) -> SentimentResult:
        ...
        return SentimentResult(
            ...
            confidence=round(confidence, 4),
            probabilities=self._score_to_probs(final_score),
            model_used=f"swahili-hybrid+{self._active_model_name}",
        )
